...
use crossterm::{
    event::{
        ...
        EnableFocusChange, EnableMouseCapture, Event, KeyEventKind, KeyModifiers,
    },
    execute,
...
use crate::tui::footer_ui::{one_line_summary, render_footer};
...
use crate::tui::subagent_routing::{
    format_task_list, open_task_pager, running_agent_count, task_mode_label,
...
use super::approval::{ApprovalMode, ReviewDecision};
use super::history::{
...
use super::views::{ConfigView, ModalKind, ShellControlView, ViewEvent};
...
use self::engine_events::EngineEventPollResult;
use self::keyboard::{KeyEventAction, OnboardingAction};
use self::terminal_events::TerminalEventOutcome;
...
async fn run_event_loop(
    ...
    loop {
        ...
        let EngineEventPollResult {
            received_engine_event,
            mut transcript_batch_updated,
            queued_to_send,
        } = engine_events::poll_engine_events(
            terminal,
            app,
            config,
            &engine_handle,
            ...
            &mut pending_translations,
            &mut pending_thinking_translations,
            &mut last_task_refresh,
            &mut last_status_frame,
            &mut force_terminal_repaint,
            draws_since_last_full_repaint,
            &mut terminal_paused_at,
        )
        .await?;
       ...
       if event::poll(poll_timeout)? {
           ...
           let key_evt = match terminal_events::handle_terminal_event(
               evt,
               terminal,
               app,
               config,
               ...
               &task_manager,
               &mut web_config_session,
               &mut last_focus_recovery,
               &mut force_terminal_repaint,
               &mut draws_since_last_full_repaint,
           )
           .await?
           {
               TerminalEventOutcome::Return => return Ok(()),
               TerminalEventOutcome::Consumed => continue,
               TerminalEventOutcome::NotConsumed(evt) => evt,
           };

           let Event::Key(key) = key_evt else {
               continue;
           };
            ...
            // Onboarding flow gets first crack at every key.
            if app.onboarding != OnboardingState::None {
                match keyboard::handle_onboarding_key(app, config, &mut engine_handle, key).await? {
                    OnboardingAction::Shutdown => return Ok(()),
                    OnboardingAction::Handled => {}
                }
                continue;
            }

            match keyboard::handle_key_event(
                terminal,
                app,
                config,
                &mut engine_handle,
            ...
            .await?
            {
                KeyEventAction::Return => return Ok(()),
                KeyEventAction::Handled | KeyEventAction::Continue => {}
            }
        }
    }
}
